<?php 
	...
	//-------------------------------------------------------------------------------------
	 if ($_GET["ff"] != "" && $_GET["gg"] != "") { 
		...
		$code = Check_Permission($conn,'กำหนดดาว PJ',$_SESSION["login_id"],"update");
    if ($code == "1") {
  ...
  if ($_GET["action"] == "apps") { 
     ...
     $appID = userSaleGroupID($conn,$_SESSION["login_id"]);
	  ...
	  if($cc == 1){
      if($appID == '') $appID = '0';
      $sql_status = "update $tbl_name set approve = '".$cc."', cs_company = '".$appID."', sign_date2 = '".date("Y-m-d")."' where $PK_field = '".$ids."'";
      @mysqli_query($conn,$sql_status);
     }else{
      if($appID == '') $appID = '0';
      $sql_status = "update $tbl_name set approve = '".$cc."', approve2 = '0', cs_company = '', cs_aceep = '', sign_date2 = '".date("Y-m-d")."', sign_date3 = '".date("Y-m-d")."' where $PK_field = '".$ids."'";
      @mysqli_query($conn,$sql_status);
     }
...
<UL class=shortcut-buttons-set>
  ...
  <LI><A class=shortcut-button href="../first_order2/index.php"><SPAN><IMG  alt=icon src="../images/icons/icon-48-section.png"><BR>
    ...
    <LI><A class=shortcut-button href="../project_order/"><SPAN><IMG  alt=icon src="../images/btn_back.gif" width="32"><BR><BR>
  กลับ</SPAN></A></LI>
    <?php  
	if ($FR_module <> "") { 
	...
	?>
      ...
      <TBODY>
        <?php  
         ...
         <TD style="vertical-align: middle;"><?php  $chaf = preg_replace("/\//","-",$rec["fs_id"]); ?><span class="text"><a href="../../upload/project_order/<?php  echo $chaf;?>.pdf" target="_blank"><?php  echo $rec["fs_id"] ; ?></a></span></TD>
         ...
         <select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu('parent',this,0)" style=" <?php  if($rec["approve"] == "0"){echo "background:#FF0;color:#000;";}elseif($rec["approve"] == "1"){echo "background:#090;color:#FFF;";}elseif($rec["approve"] == "3"){echo "background:#C60;color:#FFF;";}else{echo "background:#F00;color:#FFF;";}?>">
          ...
          <TD style="vertical-align: middle;"><div align="center"><a href="../../upload/project_order/<?php  echo $chaf;?>.pdf" target="_blank"><img src="../images/icon2/download_f2.png" width="20" height="20" border="0" alt=""></a></div></TD>
