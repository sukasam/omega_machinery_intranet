<?php    
	...
	if($_GET['action'] == 'getspareoptions'){
		...
		while($row_spare = @mysqli_fetch_array($qucgspare)){
			echo '<option value="'.$row_spare['group_id'].'">'.$row_spare['group_name'].'</option>';
		}
	}
	...
	if($_GET['action'] == 'getcus_filtered'){
		...
		if($filter_type == 'first'){
			...
			$qu_cus = mysqli_query($conn,"SELECT fo_id,cd_name,loc_name,cusid,fs_id FROM s_first_order WHERE 1 ".$consd." AND (status_use = '3' OR status_use = '0') and separate = 0 ORDER BY cd_name ASC");
			while($row_cusx = @mysqli_fetch_array($qu_cus)){
				?>
				 <tr>
			...
			// Service Order - s_first_order table (same as first order for now, but can be modified)
			$qu_cus = mysqli_query($conn,"SELECT fo_id,cd_name,loc_name,cusid,fs_id FROM s_first_order WHERE 1 ".$consd." AND (status_use = '3' OR status_use = '0') and separate = 1  ORDER BY cd_name ASC");
			while($row_cusx = @mysqli_fetch_array($qu_cus)){
				?>
				 <tr>
					<td><A href="javascript:void(0);" onclick="get_customer('<?php echo $row_cusx['fo_id'];?>','<?php echo $row_cusx['cd_name'];?>');"><?php echo $row_cusx['fs_id'];?> | <?php echo $row_cusx['cd_name'];?> (<?php echo $row_cusx['loc_name']?>)</A></td>
			...
			$qu_cus = mysqli_query($conn,"SELECT fo_id,cd_name,loc_name,cusid,fs_id FROM s_project_order WHERE 1 ".$consd." AND (status_use != '2') ORDER BY cd_name ASC");
			while($row_cusx = @mysqli_fetch_array($qu_cus)){
				?>
				 <tr>
					<td><A href="javascript:void(0);" onclick="get_customer('<?php echo $row_cusx['fo_id'];?>','<?php echo $row_cusx['cd_name'];?>');"><?php echo $row_cusx['fs_id'];?> | <?php echo $row_cusx['cd_name'];?> (<?php echo $row_cusx['loc_name']?>)</A></td>
