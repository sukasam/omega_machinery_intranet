<?php 
	...
	if ($_POST["mode"] <> "") { 
		...
		if ($_POST["mode"] == "add") { 
			...
			foreach($codes as $a => $b){
				...
				if($lists[$a] != ""){
					...
					@mysqli_query($conn,"INSERT INTO `s_service_report4sub` (`r_id`, `sr_id`, `codes`, `lists`, `units`, `prices`, `amounts`, `opens`, `remains`) VALUES (NULL, '".$id."', '".$codes[$a]."', '".$lists[$a]."', '".$units[$a]."', '".$prices[$a]."', '".$amounts[$a]."', '".$opens[$a]."', '0');");
		...
		if ($_POST["mode"] == "update" ) {
			...
			$sql2 = "select * from s_service_report4sub where sr_id = '".$_REQUEST[$PK_field]."'";
  ...
  <form action="update.php" method="post" enctype="multipart/form-data" name="form1" id="form1"  onSubmit="return check(this)">
    ...
    <center>
      ...
      <tr>
        ...
        <tr >
          <td colspan="6" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding-top:10px;padding-bottom:10px;text-align:center;"><strong>ใช้จ่ายรวม (รวมมูลค่าอะไหล่ที่เบิก)</strong></td>
        ...
        <tr >
          <td colspan="6" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding-top:10px;padding-bottom:10px;text-align:center;"><strong>ใช้จ่ายรวม (ค่าอะไหล่และอื่นๆ (จากช่างค่าน้ำมัน, ค่าทางด่วน, ที่พัก))</strong></td>
          <td colspan="3" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding-top:10px;padding-bottom:10px;text-align:right;"><strong>บาท</strong></td>
        </tr>
      </table>
        	...
        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
				...
				<select name="cs_sell" id="cs_sell" class="inputselect" style="width:50%;">
                    <?php   
						$qu_custec = @mysqli_query($conn,"SELECT * FROM s_group_technician WHERE 1 AND (group_id = 12 OR group_id = 30 OR group_id = 27)  ORDER BY group_name ASC");
						while($row_custec = @mysqli_fetch_array($qu_custec)){
							?>
