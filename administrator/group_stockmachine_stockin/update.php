<?php    
	...
	if ($_POST["mode"] <> "") { 
		...
		if ($_POST["mode"] == "add") { 
				...
				for($i=0;$i<=count($_POST['cpro']);$i++){
					if($_POST['cpro'][$i] != ""){
						...
						@mysqli_query($conn,"INSERT INTO `group_stockmachine_bill_pro` (`id`, `id_bill`, `sparpart_id`, `sparpart_qty`) VALUES (NULL,'".$id."','".$_POST['cpro'][$i]."','".$_POST['camount'][$i]."');");
						
						@mysqli_query($conn,"UPDATE `group_stockmachine` SET `group_stock` = `group_stock`+'".$_POST['camount'][$i]."' WHERE `group_id` = '".$_POST['cpro'][$i]."';");
					}
				}
			...
			$mpdf->Output('../../upload/stockin_machine/'.$chaf.'.pdf','F');
		...
		if ($_POST["mode"] == "update" ) { 
				...
				$sql2 = "select * from group_stockmachine_bill_pro where id_bill = '".$_REQUEST[$PK_field]."'";
				$quPro = @mysqli_query($conn,$sql2);
				while($rowPro = mysqli_fetch_array($quPro)){
					@mysqli_query($conn,"UPDATE `group_stockmachine` SET `group_stock` = `group_stock`-'".$rowPro['sparpart_qty']."' WHERE `group_id` = '".$rowPro['sparpart_id']."';");
				}
			
				@mysqli_query($conn,"DELETE FROM `group_stockmachine_bill_pro` WHERE `id_bill` = '".$_REQUEST[$PK_field]."'");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:12px;text-align:center;">
    ...
    <?php    
		$sub_id = $_GET['sub_id'];
		$quQry = mysqli_query($conn,"SELECT * FROM `group_stockmachine_bill_pro` WHERE id_bill = '".$sub_id."' ORDER BY id ASC");
		...
		if($_GET['mode'] == "add"){
			...
			while($rowPro = mysqli_fetch_array($quQry)){
				...
				<tr>
				  <td style="border:1px solid #000000;padding:5;text-align:center;"><?php   echo $rowCal;?></td>
				  <td style="border:1px solid #000000;padding:5;text-align:center;" id="ccode<?php   echo $rowCal;?>">
				  <?php   echo get_stockmachine_id($conn,$rowPro['sparpart_id']);?>
				  </td>
				  <td style="border:1px solid #000000;text-align:left;padding:5;">
				  <select name="cpro[]" id="cpro<?php     echo $rowCal;?>" class="inputselect" style="width:90%;" onchange="changeSpar('<?php   echo $rowCal;?>');">
						<option value="">กรุณาเลือกรายการ</option>
    ...
    <!-- <tr>
      <td colspan="3" style="text-align: right;border: 1px solid #000000;padding: 5;vertical-align: middle;font-size: 15px;font-weight: bold;">จำนวนเงินรวมทั้งสิ้น</td>
