<?php    
	...
	if($_GET['action'] == 'getcus'){
		...
		$qu_cus = mysqli_query($conn,"SELECT fo_id,cd_name,loc_name,cusid FROM s_first_order WHERE 1 ".$consd." AND (status_use = '3'  OR status_use = '0') AND `fs_id` NOT LIKE 'SV' ORDER BY cd_name ASC");
	...
	if($_GET['action'] == 'getsparactive'){
		...
		$res_spares = '|'.$row_spare['group_spar_id'].'|'.$selclist.'|'.$row_spare['group_stock'].'|'.$row_spare['group_location'];
		echo $res_spares;
	}
