<?php  
	...
	if($sr_ctype != ""){
		...
		$condition .= " AND (sv.sr_ctype = '8' OR sv.sr_ctype = '37' OR sv.sr_ctype = '38' OR sv.sr_ctype = '39' OR sv.sr_ctype = '97' OR sv.sr_ctype = '100' OR sv.sr_ctype = '101' OR sv.sr_ctype = '102')";
...
ประเภทบริการ : ( <?php  if($sr_ctype){echo get_servicename($conn,$sr_ctype);}else{echo "ทั้งหมด";}?> )
	  ...
	  $sql = "SELECT fr.*,sv.loc_contact,sv.sr_ctype,sv.sr_ctype2,sv.sv_id FROM s_first_order as fr, s_service_report as sv WHERE sv.cus_id = fr.fo_id ".$condition." ".$daterriod." GROUP BY sv.sv_id ORDER BY fr.cd_name ASC";
...
//				}
              ...
              <?php  if($_REQUEST['sh10'] == 1){?><td><?php  echo get_technician_name($conn,$row_fr['cs_company']); ?></td>  <?php  }?>    
