<?php    
	...
	if ($_GET["action"] == "apps") { 
	   ...
	   header("Location:update_bill.php?mode=update&sr_id=".$ids);
...
<form name="form1" method="get" action="index_bill.php">
    ...
    <?php    
			...
			$param2 = get_param($a_param,$a_not_exists);
			  ?>
    <a href="index_bill.php?<?php     echo $param2;?>">แสดงทั้งหมด</a>
    <?php     
			/*$a_not_exists = array();
  ...
  <div style="float:right;margin-right:20px;padding-top:5px;">  
    ...
    <select name="catalog_master" id="catalog_master" style="height:24px;" onChange="MM_jumpMenu('parent',this,0)">
		<option value="index_bill.php?app_id=0" <?php  if($_GET['app_id'] == 0){echo "selected";}?>>รอการอนุมัติ</option>
		 <option value="index_bill.php?app_id=1" <?php  if($_GET['app_id'] == 1){echo "selected";}?>>อนุมัติ</option>
         <option value="index_bill.php?app_id=2" <?php  if($_GET['app_id'] == 2){echo "selected";}?>>ไม่อนุมัติ</option>
  	</select>
    </div>
<DIV class=clear>
      ...
      <TBODY>
        ...
        <TR>
		  ...
		  <select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu('parent',this,0)" style=" <?php  if($rec["approve"] == "0"){echo "background:#FF0;color:#000;";}elseif($rec["approve"] == "1"){echo "background:#090;color:#FFF;";}elseif($rec["approve"] == "3"){echo "background:#C60;color:#FFF;";}else{echo "background:#F00;color:#FFF;";}?>">
              <option value="index_bill.php?action=apps&cc=0&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET["page"];?>" <?php  if($rec["approve"] == "0"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">รอการอนุมัติ</option>
              <option value="index_bill.php?action=apps&cc=1&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET["page"];?>" <?php  if($rec["approve"] == "1"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">อนุมัติ</option>
              <option value="index_bill.php?action=apps&cc=2&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET["page"];?>" <?php  if($rec["approve"] == "2"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">ไม่อนุมัติ</option>
              <!--<option value="index_bill.php?action=apps&cc=3&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET["page"];?>" <?php  if($rec["approve"] == "3"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">จ่ายแล้ว</option>-->
            </select>
		  </center>
		  </TD>
		  ...
		  <center><A title="view" href="../../upload/bill_lading/<?php echo $chaf;?>.pdf" target="_blank"><IMG alt="view" src="../images/icon2/backup.png" width="25"></A></center></TD>
		  </TR>  
		<?php     }?>
      </TBODY>
    </TABLE>
