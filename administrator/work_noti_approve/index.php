<?php    
  ...
  if ($_GET["action"] == "apps") { 
     ...
     $appID = userSaleGroupID($conn,$_SESSION["login_id"]);
	   ...
	   if($cc == 1){
      if($appID == '') $appID = '0';
      $sql_status = "update $tbl_name set approve = '".$cc."', sign_work2 = '".$appID."', sign_date_work2 = '".date("Y-m-d")."' where $PK_field = '".$ids."'";
      @mysqli_query($conn,$sql_status);
     }else{
      if($appID == '') $appID = '0';
      $sql_status = "update $tbl_name set approve = '".$cc."', approve2 = '".$cc."', sign_work2 = '', sign_work3 = '', sign_date_work2 = '".date("Y-m-d")."', sign_date_work3 = '".date("Y-m-d")."' where $PK_field = '".$ids."'";
      @mysqli_query($conn,$sql_status);
     }
