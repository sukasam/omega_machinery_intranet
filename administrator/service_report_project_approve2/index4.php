<?php 
	...
	//-------------------------------------------------------------------------------------
	 ...
	 if ($_GET["action"] == "apps") { 
		 ...
		 $sql_status = "update $tbl_name set approve_gm = '".$cc."', loc_contact4 = '".$appID."' where $PK_field = '".$ids."'";
		 @mysqli_query($conn,$sql_status);

		 header("Location:update4.php?mode=update&sr_id=".$ids);
...
<UL class=shortcut-buttons-set>
	<LI><A class=shortcut-button href="../service_report7/"><SPAN><IMG alt=icon src="../images/btn_back.gif"><BR><BR>
	กลับ<BR><BR></SPAN></A></LI>
	<!-- <LI><A class=shortcut-button href="../service_report_project_approve/index2.php"><SPAN><IMG  alt=icon src="../images/icons/icon-48-section.png"><BR>
    ใบเบิกอะไหล่ - โปรเจ็ค</SPAN></A></LI>
    <LI><A class=shortcut-button href="../service_report_project_approve/index3.php"><SPAN><IMG  alt=icon src="../images/icons/icon-48-section.png"><BR>
    ใบยืมอะไหล่ - โปรเจ็ค</SPAN></A></LI>
    <LI><A class=shortcut-button href="../service_report_project_approve/index5.php"><SPAN><IMG  alt=icon src="../images/icons/icon-48-section.png"><BR>
    ใบคืนอะไหล่ - โปรเจ็ค</SPAN></A></LI> -->
    <LI><A class=shortcut-button href="../service_report_project_approve2/index4.php"><SPAN><IMG  alt=icon src="../images/icons/icon-48-section.png"><BR>
    ใบรายงานติดตั้ง - โปรเจ็ค</SPAN></A></LI>
    <?php  
	if ($FR_module <> "") { 
	...
	?>
      ...
      <TBODY>
        <?php  
					...
					if ($_GET['app_id'] <> "") { 
						$sql .= " and ( sr.approve_gm = '".$_GET["app_id"]."' ";
						$sql .=  $subtext . " ) ";
					}else{
						$sql .= " and ( sr.approve_gm = '0' ";
						$sql .=  $subtext . " ) ";
					}
        ...
        <TR>
          ...
          <TD style="vertical-align:middle"><select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu('parent',this,0)" style=" <?php  if($rec["approve_gm"] == "0"){echo "background:#FF0;color:#000;";}elseif($rec["approve_gm"] == "1"){echo "background:#090;color:#FFF;";}elseif($rec["approve_gm"] == "2"){echo "background:#C60;color:#FFF;";}else{echo "background:#F00;color:#FFF;";}?>">
            <option value="index4.php?action=apps&cc=0&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET["page"];?>" <?php  if($rec["approve_gm"] == "0"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">รอการอนุมัติ</option>
            <option value="index4.php?action=apps&cc=1&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET["page"];?>" <?php  if($rec["approve_gm"] == "1"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">อนุมัติ</option>
            <option value="index4.php?action=apps&cc=2&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET["page"];?>" <?php  if($rec["approve_gm"] == "2"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">ไม่อนุมัติ</option>
           <!-- <option value="index4.php?action=apps&cc=3&sr_id=<?php  echo $rec["sr_id"];?>&page=<?php  echo $_GET["page"];?>" <?php  if($rec["approve_gm"] == "3"){echo 'selected="selected"';}?> style="background:#FFF;color:#000;">จ่ายแล้ว</option>-->
          </select></TD>
