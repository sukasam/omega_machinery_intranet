<?php 
	...
	if ($_POST["mode"] <> "") { 
		...
		if ($_POST["mode"] == "add") { 
			...
			foreach($codes as $a => $b){
				...
				if($lists[$a] != ""){
					...
					@mysqli_query($conn,"INSERT INTO `s_service_report8sub` (`r_id`, `sr_id`, `codes`, `lists`, `units`, `prices`, `amounts`, `opens`, `remains`) VALUES (NULL, '".$id."', '".$codes[$a]."', '".$lists[$a]."', '".$units[$a]."', '".$prices[$a]."', '".$amounts[$a]."', '".$opens[$a]."', '0');");
			...
			include("../service_report8/form_serviceopen.php");
			$mpdf=new mPDF('UTF-8'); 
			$mpdf->SetAutoFont();
			$mpdf->WriteHTML($form);
		...
		if ($_POST["mode"] == "update" ) {
			...
			$sql2 = "select * from s_service_report8sub where sr_id = '".$_REQUEST[$PK_field]."'";
			...
			@mysqli_query($conn,"DELETE FROM `s_service_report8sub` WHERE `sr_id` = '".$_REQUEST[$PK_field]."'");
	  ...
	  <tr>
		<td style="text-align:right;font-size:12px;">
			<div style="position:relative;text-align:center;">
            	<img src="../images/form/header_service_report8.png" width="100%" border="0" style="max-width:1182px;"/>
            </div>
		</td>
	  </tr>
	</table>
...
&nbsp;&nbsp;<strong>เลขที่ใบงาน</strong> : <input type="text" name="srid" value="<?php   echo $srid;?>" id="srid" class="inpfoder">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="dataTable" style="text-align:center;margin-top:5px;">
      ...
      <tbody>
        <?php   
			$qu = @mysqli_query($conn,"SELECT * FROM s_service_report8sub WHERE sr_id = '".$sr_id."' AND sr_id != '0'  ORDER BY r_id ASC");
