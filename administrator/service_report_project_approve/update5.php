<?php 
	...
	if ($_POST["mode"] <> "") { 
		...
		if ($_POST["mode"] == "update" ) {
			...
			$sql2 = "select * from s_service_report10sub where sr_id = '".$_REQUEST[$PK_field]."'";
			...
			$sql3 = "select * from s_service_report10sub where sr_id = '".$_REQUEST[$PK_field]."'";
			$quPro3 = @mysqli_query($conn,$sql3);
			
			@mysqli_query($conn,"DELETE FROM `s_service_report10sub` WHERE `sr_id` = '".$_REQUEST[$PK_field]."'");
			...
			while($rowPro3 = mysqli_fetch_array($quPro3)){
				...
				@mysqli_query($conn,"INSERT INTO `s_service_report10sub` (`r_id`, `sr_id`, `codes`, `lists`, `units`, `prices`, `amounts`, `opens`, `remains`) VALUES (NULL, '".$rowPro3['sr_id']."', '".$rowPro3['codes']."', '".$rowPro3['lists']."', '".$rowPro3['units']."', '".$rowPro3['prices']."', '".$rowPro3['amounts']."', '".$rowPro3['opens']."', '".$remains[$runA]."');");
...
////					echo $remains[$a];
//					
//					@mysqli_query($conn,"INSERT INTO `s_service_report10sub` (`r_id`, `sr_id`, `codes`, `lists`, `units`, `prices`, `amounts`, `opens`, `remains`) VALUES (NULL, '".$id."', '".$codes[$a]."', '".$lists[$a]."', '".$units[$a]."', '".$prices[$a]."', '".$amounts[$a]."', '".$opens[$a]."', '".($amounts[$a]-$opens[$a])."');");
...
//			exit();
//			foreach($codes as $a => $b){
//				$resupdate = get_plusminus2($conn,"s_group_sparpart","s_service_report10sub",$rid[$a],$lists[$a]);
//				
//				@mysqli_query($conn,"UPDATE `s_service_report10sub` SET `codes` = '".$codes[$a]."', `lists` = '".$lists[$a]."', `units` = '".$units[$a]."', `prices` = '".$prices[$a]."', `opens` = '".$opens[$a]."', `remains` = '".$remains[$a]."' WHERE `r_id` =".$rid[$a]."");
//				
...
//				
//				@mysqli_query($conn,"UPDATE `s_service_report10sub` SET `amounts` = '".$amount['group_stock']."' WHERE `r_id` = '".$rid[$a]."';");	
//				
//				$amountss[] = $amount['group_stock'];
//				/*if($opens[$a] == 0){
//					@mysqli_query($conn,"UPDATE `s_service_report10sub` SET `codes` = '', `lists` = '', `units` = '', `prices` = '', `amounts` = '', `opens` = '', `remains` = '' WHERE `r_id` =".$rid[$a]."");
//				}*/
//			}	
			...
			include("../service_report10/form_serviceopen.php");
			$mpdf=new mPDF('UTF-8'); 
			$mpdf->SetAutoFont();
			$mpdf->WriteHTML($form);
			...
			header ("location:index5.php?" . $param); 
...
<DIV id=tab1 class="tab-content default-tab">
  <form action="update5.php" method="post" enctype="multipart/form-data" name="form1" id="form1"  onSubmit="return check(this)">
    <div class="formArea">
      <fieldset>
	  ...
	  <tr>
		<td style="text-align:right;font-size:12px;">
			<div style="position:relative;text-align:center;">
            	<img src="../images/form/header_service_report10.png" width="100%" border="0" style="max-width:1182px;"/>
            </div>
		</td>
	  </tr>
	</table>
...
<input type="text" name="sv_id" value="<?php   if($sv_id == ""){echo check_serviceman2($conn);}else{echo $sv_id;};?>" id="sv_id" class="inpfoder" style="border:0;"><!--<input type="text" name="sv_id" value="<?php   if($sv_id == ""){echo "SR";}else{echo $sv_id;};?>" id="sv_id" class="inpfoder" style="border:0;">&nbsp;&nbsp;เลขที่สัญญา  :</strong> <span id="contactid"><?php   echo $finfo['fs_id'];?></span>--></td>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb1">
      <tr>
        ...
        <textarea name="detail_recom" class="inpfoder" id="detail_recom" style="width:50%;height:100px;background:#FFFFFF;font-size:14px;font-family:Verdana, Geneva, sans-serif;"><?php   echo strip_tags($detail_recom);?></textarea></td>
      </tr>
    </table>
    
    <center>
      <br>
      <span style="font-size:18px;font-weight:bold;">รายละเอียดการยืมอะไหล่ เพื่อติดตั้ง</span></center><br>
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="dataTable" style="text-align:center;margin-top:5px;">
      <tr>
        ...
        <?php   
		 $qu = @mysqli_query($conn,"SELECT * FROM s_service_report10sub WHERE sr_id = '".$sr_id."' ORDER BY r_id ASC");
        ...
        <tr >
			<td colspan="3">
				หมายเหตุ
				  ...
				  <td colspan="3" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding-top:10px;padding-bottom:10px;text-align:center;"><strong>รวมจำนวนที่เบิก</strong></td>
				  <td colspan="2" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding-top:10px;padding-bottom:10px;text-align:right;"><strong>รายการ</strong></td>
				  </tr>
        <tr >
			<td colspan="3">
				<textarea name="detail_comment" id="detail_comment<?php   echo $i;?>" style="width:100%;height:100px;background:#FFFFFF;font-size:14px;font-family:Verdana, Geneva, sans-serif;"><?php   echo strip_tags($detail_comment);?></textarea>
			</td>
          <td colspan="3" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding-top:10px;padding-bottom:10px;text-align:center;"><strong>ใช้จ่ายรวม (รวมมูลค่าอะไหล่ที่เบิก)</strong></td>
          <td colspan="2" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;padding-top:10px;padding-bottom:10px;text-align:right;"><strong>บาท</strong></td>
          </tr>
    </table>
	  ...
	  <tr>
        ...
        <td width="33%" style="border:1px solid #000000;font-size:12px;font-family:Verdana, Geneva, sans-serif;text-align:center;padding-top:10px;padding-bottom:10px;">
        	<table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td style="padding-top:10px;padding-bottom:10px;font-size:12px;font-family:Verdana, Geneva, sans-serif;text-align:center;"><strong>ผู้อนุมัติ - หัวหน้างานฝ่ายโปรเจ็ค</strong></td>
              </tr>
              <tr>
                <td style="font-size:12px;font-family:Verdana, Geneva, sans-serif;text-align:center;"><strong>วันที่ :</strong>
