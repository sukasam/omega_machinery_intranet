...
if ($_GET["action"] == 'getprodetail') {
	...
	if ($fsid == "SV") {
		echo '<input type="text" name="loc_pro" value="' . get_proname2($conn, $prolistname[$pid]) . '" id="loc_pro" class="inpfoder" style="width:50%;">|<input type="text" name="loc_seal" value="' . $prolistpod[$pid] . '" id="loc_seal" class="inpfoder" style="width:20%;">|<input type="text" name="loc_sn" value="' . $prolistsn[$pid] . '" id="loc_sn" class="inpfoder" style="width:20%;">';
	} else {
...
if ($_GET["action"] == 'getcusfirsh') {
	...
	$plid = "<select name=\"bbfpro\" id=\"bbfpro\" onchange=\"get_podsn(this.value,'lpa1','lpa2','lpa3','" . $fpid . "','" . substr($rowcus['fs_id'], 0, 2) . "')\">
						<option value=\"\"><<== Select ==>></option>       
					 ";

	if (substr($rowcus['fs_id'], 0, 2) == "SV") {
	...
	$displ = $rowcus['cd_address'] . "|" . province_name($conn, $rowcus['cd_province']) . "|" . $rowcus['cd_tel'] . "|" . $rowcus['cd_fax'] . "|" . $rowcus['fs_id'] . "|" . format_date($rowcus['date_quf']) . "||" . $rowcus['c_contact'] . "|" . $rowcus['c_tel'] . "|" . $rowcus['loc_name'] . "|" . get_lastservice_s($conn, $fpid, "") . '|' . $plid . '|' . $ctyp . '|' . $feeder . '|' . $rowcus['garun_id'] . '|' . $rowcus['date_quf'] . '|' . $rowcus['date_qut'];
	echo $displ;
}
