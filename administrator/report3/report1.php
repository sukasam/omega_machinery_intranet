...
รายงานการรับสินค้าเข้าสต๊อค</th>
      ...
      <?php  
		...
		$dbservice = "group_stockmachine_bill";

		$dbservicesub = "group_stockmachine_bill_pro";
              ...
              <?php  if($_REQUEST['sh4'] == 1 || $_REQUEST['sh5'] == 1 || $_REQUEST['sh8'] == 1){?><td style="padding:0;">
				...
				while($row = @mysqli_fetch_array($qu_pfirst)){
					...
					<tr>
					  ...
					  <?php  if($_REQUEST['sh4'] == 1){?><td style="border-bottom:none;" width="25%"><?php  echo get_stockmachine_id($conn,$row['sparpart_id']);?></td><?php  }?>

					  <?php  if($_REQUEST['sh5'] == 1){?><td align="left" style="border-bottom:none;" width="50%"><?php  echo get_stockmachine_name($conn,$row['sparpart_id']);?></td><?php  }?>

					  <?php  if($_REQUEST['sh6'] == 1){?><td align="center" style="border-bottom:none;" width="25%"><?php  echo $row['sparpart_qty'];?></td><?php  }?>
